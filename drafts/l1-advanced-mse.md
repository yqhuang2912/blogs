...
在上一篇《[为什么生成模型要使用MSE？](generative-model-mse.html)》中，我们从概率建模的角度推导了一个结论：如果假设误差:$x = f_\theta(y) + \epsilon$中的噪声$\epsilon$服从高斯分布，那么最大化似然就等价于最小化$(x - f_\theta(y))^2$，也就是 **MSE 损失**。
