...
我们经常会想要知道两个分布$P$和$Q$到底有多像？如果它们都是高斯，比如$P = \mathcal{N}(\mu_1, \sigma_1^2)$，$Q = \mathcal{N}(\mu_2, \sigma_2^2)$，我们可以通过比较它们的均值和方差来判断它们的相似性。但如果两个分布形式完全不同，比如$P$是高斯分布，$Q$是泊松分布，靠「对比参数」就没法统一度量了。

这时候就会自然地问一句：有没有一种统一的刻度，能在同一把尺子上衡量不同分布？答案是有的，
- 对于「单个系统本身的不确定性」：有把尺子，叫**熵（entropy）**。
- 对于「两个分布之间的差异」：有另外一把尺子，叫**相对熵（relative entropy）**，也叫**KL散度（Kullback-Leibler divergence）**。

## 信息量：单个事件带来多少“确定性”
对于一个随机事件，它究竟有多少信息量？直觉上有几个要求：
- **越不可能的事情，信息量越大**。如果这个事件很罕见，发生的概率很小，那么当它发生时所带来的信息量应该很大，因为它让我们对世界的认知发生了很大的改变。如果这个事件很常见，发生的概率很大，那么当它发生时所带来的信息量应该很小，因为它对我们的认知几乎没有影响，比如：
...
从前面对信息量的直观理解我们知道，信息量是和事件发生的概率息息相关的，所以我们可以假设信息量是概率的某个函数$f(\cdot)$：
$$
I(A) := f(P(A)) \tag{3}
$$
注意这里是$:=$表示这个式子是我们定义的，而不是从别的式子推导出来的。结合式(1-3)，我们可以得到：
$$
\begin{aligned}
    I(\text{阿根廷夺冠}) &= I(A) + I(B) \\
...
这里的**信息量可以理解成一个事件，从原来的不确定变得确定的难度有多大**，它只和先验概率$p(x)$有关，和其他因素无关，例如：
...
设随机变量$X$的取值范围为$\{x_1, x_2, \ldots\}$，对应的概率分别为$P(X=x_i) = p_i$，那么在这个系统分布下，平均信息量就是：
$$H(X) = \sum_{i=1} p_i (-\log_2 p_i) = \mathbb{E}_{x\sim P(X)} [-\log_2 P(x)] \tag{12}$$
...
$$H(P) = \mathbb{E}_{x\sim P(x)} [-\log_2 P(x)] \tag{15}$$

1. **实际使用的平均代价**：真实情况是我们并不知道真实分布，只能用假设分布$Q$来编码，平均代价是交叉熵：
$$H(P, Q) = \mathbb{E}_{x\sim P(x)} [-\log_2 Q(x)] \tag{16}$$

这就是**交叉熵 (cross-entropy)** 的定义，它衡量的是：**如果数据是从真实分布$P$来的，但我们用假设分布$Q$去编码数据，平均要付出的信息量/编码长度。**

## KL 散度：两个分布之间的多余信息量
现在终于到 KL 散度登场了。最理想的平均代价是熵$H(P)$，实际使用的平均代价是交叉熵$H(P, Q)$，那么两者的差值就是因为使用了<span style="color:red;">“错误”</span>分布$Q$而多付出的代价，这个差值就是**KL 散度 (Kullback-Leibler divergence)**：
$$
\begin{aligned}
    D_{KL}(P \| Q) &= H(P, Q) - H(P) \\
    &= \mathbb{E}_{x\sim P(X)} [-\log_2 Q(x)] - \mathbb{E}_{x\sim P(X)} [-\log_2 P(x)] \\
    &= \mathbb{E}_{x\sim P(X)} \left[\log_2 \frac{P(x)}{Q(x)}\right]
\end{aligned}
\tag{17}$$
直观上我们可以这样理解KL散度：
...
此时比较两个分布的统一刻度就出来了：并不是单独比较熵，而是比较对应的 KL 散度$D_{KL}(P \| Q)$。
...
$$
例如对于分类问题，我们假设$x$是输入数据，$y$是标签，则条件分布$P_{\text{data}}(y|x)$和$Q_{\theta}(y|x)$才是我们关心的分布，所以最小化KL散度的目标变成了：
$$
...
\end{aligned} \tag{21}$$
从式(21)也可以看出：**交叉熵和KL散度之间只差一个和参数无关的常数$H(P_{\text{data}}(y|x))$**。注意到最后一项$H(P_{\text{data}}(y|x))$和参数$\theta$无关，所以我们可以把它忽略掉，最终的优化目标就变成了最小化交叉熵：
$$
\theta^{*} = \arg\min_{\theta} H(P_{\text{data}}(y|x), Q_{\theta}(y|x)) \tag{22}
$$
这就是为什么在机器学习中我们经常使用交叉熵作为损失函数的原因，因为它等价于最小化KL散度，从而让假设分布尽可能接近真实分布。所以在训练时，我们做的就是用样本$(x_n, y_n)$来估计交叉熵：
$$
\begin{aligned}
    H(P_{\text{data}}(y|x), Q_{\theta}(y|x)) &= \mathbb{E}_{(x,y)\sim P_{\text{data}}} \left[- \log_2 Q_{\theta}(y|x)\right]\\
    & \approx - \frac{1}{N} \sum_{n=1}^{N}P_{\text{data}}(y_n|x_n) \log_2 Q_{\theta}(y|x_n)
\end{aligned} \tag{23}
$$
然后通过梯度下降法最小化这个估计值，从而让分布$Q_{\theta}(y|x)$尽可能接近真实分布$P_{\text{data}}(y|x)$。Loss函数通常写成：
$$
\mathcal{L}_{\text{cls}} = - \frac{1}{N} \sum_{n=1}^{N} y_n \log_2 \hat{y}_n \tag{24}
$$
这里的$y_n$是one-hot编码的真实标签，$\hat{y}_n = Q_{\theta}(y|x_n)$是模型预测的概率分布。
> 这里有个问题：label和prediction的顺序可以颠倒吗？也就是说，能不能用下面的Loss函数呢？
> $$
\mathcal{L}_{\text{cls}}^{'} = - \frac{1}{N} \sum_{n=1}^{N} \hat{y}_n \log_2 y_n \tag{25}
$$
> 答案是不行的，因为交叉熵是**不对称的**，也即$H(P, Q) \ne H(Q, P)$，所以式(24)和式(25)是完全不同的两个Loss函数。实际上，式(25)在实际应用中是没有意义的，因为真实标签$y_n$是one-hot编码的分布，它只有一个类别的概率为1，其他类别概率为0，$\log(0)$是没有意义的，所以用式(25)计算出来的Loss基本上是无穷大，没有任何参考价值。
> 
> 交叉熵损失在Pytorch的实现中为`loss = F.cross_entropy(input, target)`，第一个参数`input`是模型预测的概率分布，第二个参数`target`是真实标签的索引（不是one-hot编码），所以我们写代码的时候要注意顺序，不要弄反了。

## 小结
我们回顾一下上一节《[概率和似然](probability-and-likelihood.html)》中介绍的**最大似然估计（MLE）**，可以发现，式(23)正好就是MLE的目标函数。所以，**用$Q_{\theta}$去逼近真实分布$P_{\text{data}}$，等价于最大化似然估计，等价于最小化交叉熵，等价于最小化KL散度**，这些目标函数本质上是一样的，只是从不同的角度去理解而已。
