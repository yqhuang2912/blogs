...
我们现在有的是什么？我们可以从现实中收集到一组训练数据集$\mathcal{D} = \{(x_n, z_n)\}_{n=1}^{N}$，这些数据是从真实分布$p_{\text{data}}(x, y): y \to f^{\ast}(y)=x$中采样得到的。我们的终极目标是找到一个好的参数$\theta$，使得模型在观测数据$y$作为条件下生成的数据$\hat{x}$就好像是从真实分布$p_{\text{data}}(x,y)$中采样得到的一样。根据之前的《[概率和似然](probability-and-likelihood.html)》一文，我们可以通过最大化似然估计来实现这个目标：
$$
