...
## 定义生成任务
假设我们有一个生成任务，比如图像超分辨率或者CT图像重建。输入观测数据$y$，这里的$y$可以是低分辨率图像，也可以是CT的sinogram数据，在真实世界中客观存在一个映射关系$f^{\ast}$，使得我们可以从$y$生成高质量的图像$x$。但是，这个真实的映射关系$f^{\ast}$通常是未知的，我们只能通过数据来学习一个近似的映射函数$f_{\theta}$，使得$f_{\theta}(y)$产生的数据$\hat{x}$尽可能接近真实的映射关系$f^{\ast}(y)$产生的真实图像$x$。

在生成模型的视角下，生成不仅仅是预测一个具体的图像，而是希望学习一个条件概率分布$p(x|y; \theta)$，表示在给定观测数据$y$的情况下，真实图像$x$的分布。

<!-- more -->

我们现在有的是什么？我们可以从现实中收集到一组训练数据集$\mathcal{D} = \{(x_i, y_i)\}_{i=1}^{N}$，这些数据是从真实分布$p_{\text{data}}(x, y): y \to f^{\ast}(y)=x$中采样得到的。我们的终极目标是找到一个好的参数$\theta$，使得模型在观测数据$y$作为条件下生成的数据$\hat{x}$就好像是从真实分布$p_{\text{data}}(x,y)$中采样得到的一样。根据之前的《[概率和似然](probability-and-likelihood.html)》一文，我们可以通过最大化似然估计来实现这个目标：
$$
...
$$
这就是生成问题的最一般化的目标函数的形式。接下来我们面临一个关键问题：**条件概率分布$p(x|y)$到底长啥样？**

## 为什么选择高斯分布？
我们换个思路：我们想要生成的图像$\hat{x}$应该和真实图像$x$尽可能接近，但是，什么叫“尽可能接近”呢？就是这两者之间的差异要尽可能小。我们使用$\epsilon$来表示这个差异，则有：
$$x = \hat{x} + \epsilon = f_{\theta}(y) + \epsilon \tag{3}$$

这意味着，$p(x|y)$的形状完全取决于$\epsilon$的分布的形状。
...
## 从高斯分布到MSE
既然基于**中心极限定理**和**最大熵原理**确定了误差$\epsilon$服从高斯分布：$\epsilon \sim \mathcal{N}(0, \sigma^2)$，那么条件概率分布$p(x|y)$也就确定下来了：
...
$$
\begin{aligned}
    \theta^{\ast} &= \arg\min_{\theta} \sum_{i=1}^{N} (x_i - f_{\theta}(y_i))^2 \\
    &= \arg\min_{\theta} \sum_{i=1}^{N} (x_i - \hat{x}_i)^2
\end{aligned}
\tag{17}
$$
即:
$$\mathcal{L}_{\text{MSE}}(\theta) = \sum_{i=1}^{N} (x_i - \hat{x}_i)^2 \tag{18}$$
这正是均方误差（MSE）损失函数的形式。