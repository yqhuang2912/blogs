...
tags:
  ...
  - 强化学习
---
我们在上一篇文章《从策略梯度到REINFORCE算法》中提到，轨迹$\tau$表示Agent在环境中经历的一系列状态、动作和奖励的序列：
...
2. **动作选择$a_t$**：Agent根据当前状态$s_t$选择动作$a_t$，如果策略是随机的，那么动作选择也是随机的。
3. **状态转移$s_{t+1}$**：环境根据当前状态$s_t$和动作$a_t$，随机地转移到下一个状态$s_{t+1}$，这个转移$P(s_{t+1}|s_t,a_t)$通常是概率性的。
...
我们定义$P_{\theta}(\tau)$是轨迹$\tau$的概率分布，它表示在策略参数为$\theta$的情况下，Agent执行出轨迹$\tau$的概率。所以我们写成：
$$\tau \sim P_{\theta}(\tau) \tag 3$$
意思就是，轨迹$\tau$是从概率分布$P_{\theta}(\tau)$中采样得到的一个随机变量。

我们把轨迹看作所有随机事件的集合，这样，每执行策略$\pi_{\theta}$一次，就会得到一条新的轨迹$\tau$样本。用随机过程的角度看：
$$\tau \sim P_{\theta}(\tau) = \rho(s_0) \prod_{t=0}^{T-1} \pi_{\theta}(a_t|s_t) P(s_{t+1}|s_t,a_t) R(r_t|s_t,a_t)$$
...
<span style="color:#e67e22">文字</span>